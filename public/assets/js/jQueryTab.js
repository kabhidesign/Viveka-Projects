  ...
  jQueryTab.prototype = {
    ...
    addHeader: function () {
      ...
      self.tabLinks.each(function () {
        ...
        $("<a />")
          .attr({
            class: self.headerClass,
            href: "#" + $tab_content.attr("id"),
          })
          .text($(this).text())
    ...
    handleClasses: function (activeContent) {
      var self = this;
      self.contents
        .removeClass(self.options.tabInTransition)
        .addClass(self.options.tabOutTransition);
      activeContent
        .removeClass(self.options.tabOutTransition)
        .addClass(self.options.tabInTransition);
    },
    ...
    resize: function () {
      ...
      if (self.options.responsive)
        self.alinks.eq(currentTab).addClass(self.activeClass);
      ...
      if (self.options.responsiveBelow < $(window).width()) {
        ...
        if (self.options.responsive) {
          self.contents
            .hide()
            .removeClass(
              self.options.tabOutTransition,
              self.options.tabInTransition
            );
    ...
    handleEvent: function () {
      ...
      self.tabLinks
        .add(self.alinks)
        .on(
          self.options.openOnhover ? "click mouseenter" : "click",
          function (event) {
            // when tab is clicked
            event.preventDefault(); // prevent default behaviour
            var fxn_to_call = $(this).hasClass("accordion_tabs")
              ? "handleAccordions"
              : "handleTabs";
            self.tabber(fxn_to_call, this);
          }
    ...
    tabber: function (fxn_name, elem) {
      var self = this;
      typeof self.options.before === "function"
        ? self.options.before.apply(elem, arguments)
        : ""; // call fxn before
      if (self.options.useHistory)
        self.updateBrowserHisotry($(elem).attr("href"));
      self[fxn_name].call(elem, self); // call handleEvent fxn; self.options - data values
      typeof self.options.after === "function"
        ? self.options.after.apply(elem, arguments)
        : ""; // call fxn after
    },
    ...
    handleTabs: function (self) {
      ...
      self.tabItems
        ...
        .eq(index)
        .addClass(self.activeClass); // add active class to this tab
      self.wrap.height(activeContent.outerHeight());
      self.reOrder(activeContent);
      if (self.options.responsive)
        self.alinks
          .removeClass(self.activeClass)
          .eq(index)
          .addClass(self.activeClass); // add class to corresponding accordian header
    ...
    handleAccordions: function (self) {
      ...
      if ($this.hasClass(self.activeClass) && !self.options.collapsible)
        return; // if active and not collapsible - do nothing
      else if ($this.hasClass(self.activeClass) && self.options.collapsible) {
        // if active and collapsible
        ...
        self.options.accordionTransition === "slide"
          ? self.contents.slideUp(outtime)
          : self.contents.fadeOut(outtime); // else hide the visible content in specified time
        ...
        self.tabItems
          .removeClass(self.activeClass)
          .eq(index)
          .addClass(self.activeClass); // add active class to the corresponding tab
    ...
    getActiveTab: function (tab) {
      ...
      if (typeof tab === "number") {
        return tab - 1;
      } // if tab specified return it immediately
      ...
      if (window.location.hash) {
        var indexOfHash = self.tabLinks.index(
    ...
    setCookie: function (value) {
      ...
      if (value === null) {
        ...
        date = new Date();
        date.setTime(
          date.getTime() + self.options.cookieExpires * 24 * 60 * 60 * 1000
        );
